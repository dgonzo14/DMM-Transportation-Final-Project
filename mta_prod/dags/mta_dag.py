...
with DAG(
    ...
    description="MTA bronze -> silver -> gold production pipeline writing to Snowflake",
...
"""
    ...
    fetch_bronze_once = BashOperator(
        ...
        + '\npython -m mta_prod.prepare_database\npython -m mta_prod.mta_producer --dag-run-id "{{ dag_run.run_id }}"',
    )

    load_bronze_to_spark_silver = BashOperator(
        task_id="load_bronze_to_spark_silver",
        bash_command=common_prefix
        + '\npython -m mta_prod.mta_silver_runner --dag-run-id "{{ dag_run.run_id }}" --bronze-type full_feed',
    )

    update_gold_metrics = BashOperator(
        task_id="update_gold_metrics",
        bash_command=common_prefix
        + '\npython -m mta_prod.mta_gold --dag-run-id "{{ dag_run.run_id }}"',
    )
